...
# March 6th 2026 10:23 pm
I added my main method that will run the entire function. I was able to run it in interactive mode but not batch mode. I had to look up on how to run it in bash, realized that I needed to add racket to my path environments and then run it that way. Even when I did that, I ran into a problem where the bash terminal would just end. So to solve this problem, I found a method called current-input-port that allows the input to only be taken through the terminal. My divison and subtraction wasn't calculated properly as well, so I had to ensure that whitespace was being accounted for.
